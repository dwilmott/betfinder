tidyup_betrivers_data <- function(betrivers_data, sport, prop = FALSE, game_lines = FALSE,
                                  key = get_key_path(sport = sport, prop = prop, game_lines = game_lines)) {


  if (nrow(betrivers_data) < 1) stop('no betrivers ', prop, ' available')

  # make the output from the input
  output_df <- betrivers_data
  ...
  if (prop %in% c('first player to score', 'fpts')) {
    split_names <- strsplit(output_df$participantName, ', ')
